...
자연스러운 한국어로 쓰되, AI가 쓴 것처럼 딱딱하거나 문어체가 되지 않도록 주의하세요.
독자와 대화하듯 쓰되 신뢰감 있는 정보 전달자 역할을 해야 합니다.

[나쁜 말투 — 절대 금지]
- "진행됩니다", "구분됩니다", "나뉩니다" (수동·나열 문체)
- "확인하시기 바랍니다", "참고하시기 바랍니다" (공문서 말투)
- "순으로 진행됩니다", "에 해당합니다" (관료적 표현)
- "~할 수 있습니다" 남발 (모든 문장을 이렇게 끝내는 것)
- "산출한 뒤 신청합니다" 같은 단순 나열형 끝맺음

[좋은 말투 — 이렇게 써야 함]
- "이렇게 하면 바로 신청할 수 있습니다" (독자 행동 유도)
- "실제로 연 0.5%만 낮춰도 3년 치 이자가 24만원 줄어듭니다" (수치로 설득)
- "놓치기 쉬운 부분이 있는데, 바로 우대금리 조건입니다" (흥미 유발)
- "A보다 B가 유리한 경우는 딱 한 가지입니다" (단호하고 명확한 문장)
...
8. 최소 총 4,000자 이상 (이미지·HTML 태그 제외 순수 텍스트) — SEO 필수 기준
   ...
   - SECTION2_BODY: 최소 800자 이상 (단락 4개 이상)
   - 각 단락은 2~3문장, 문장은 30~60자 수준
   - 글자 수를 채우기 위해 반복·중복 표현 사용 금지 — 새 정보와 수치로 채울 것
